...
void tearDown(void)
{
    // Left intentional for future tear downs
}

// --- U8G2 GRAPHICS ENGINE SETUP LOWER-LAYER INTERCEPT & OVERRIDE MOCKS ---
void __wrap_u8g2_Setup_sh1106_i2c_128x64_noname_f(u8g2_t* u8g2, const u8g2_cb_t* rotation, u8x8_msg_cb byte_cb,
...
void __wrap_u8x8_InitDisplay(u8x8_t* u8x8) { (void)u8x8; }

void __wrap_u8x8_SetPowerSave(u8x8_t* u8x8, uint8_t is_enable)
{
    (void)u8x8;
    (void)is_enable;
}

void __wrap_u8g2_ClearBuffer(u8g2_t* u8g2) { (void)u8g2; }
...
void test_driver_initialization_registers_callbacks(void)
{
    u8g2_t u8g2;
    I2C_HandleTypeDef fake_hi2c;

    printf("[TRACE] Starting initialization test...\n");
    memset(&u8g2, 0, sizeof(u8g2_t));
    memset(&fake_hi2c, 0, sizeof(I2C_HandleTypeDef));

    fake_hi2c.Instance = &host_i2c;
    mock_hal_reset();

    printf("[TRACE] Calling U8G2_HAL_SH1106_Init...\n");
    U8G2_HAL_SH1106_Init(&u8g2, &fake_hi2c);
    printf("[TRACE] Returned from U8G2_HAL_SH1106_Init successfully!\n");

    printf("[TRACE] Asserting callbacks...\n");
    // Verify that the driver successfully registered your custom DMA I2C callbacks into the structure
    TEST_ASSERT_NOT_NULL_MESSAGE(u8g2.u8x8.byte_cb, "Callback Error: U8g2 Byte callback was not successfully bound!");
    TEST_ASSERT_NOT_NULL_MESSAGE(u8g2.u8x8.gpio_and_delay_cb,
                                 "Callback Error: U8g2 GPIO/Delay callback was not successfully bound!");

    printf("[TRACE] Initialization test finished cleanly!\n");
}
...
int main(void)
{
    ...
    RUN_TEST(test_driver_initialization_registers_callbacks);
    return UNITY_END();
}
